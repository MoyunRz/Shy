...
function getAreaBuy(val,val1){
	...
	newStr.push("<li onclick=\"getCityBuy("+val1+")\" style=\"background-color:#ccc;padding-left:20px;\">"+city[val].NAME+"</li>");
	...
	if(asize==0){
		...
		$("#endaddress").attr({"SS":province[val1].NAME,"SQ":city[val].NAME,"XS":""});
		$("#endaddress").val(allarea);
		$("body .dqld_div").remove();
	}
	else{
		$("body .dqld_div").remove();
...
function getallArea(val,val1,val2){
	...
	$("#endaddress").attr({"SS":province[val].NAME,"SQ":city[val1].NAME,"XS":area[val2].NAME});
	$("#endaddress").val(allarea);
	$("body .dqld_div").remove();
}
/*地区联动*/